...
def compute_summary(df):
    """Group-level summary: mean/std error per (subject, session, finger, zone)."""
    summary = (
...
def extract_landmark_positions(df):
    ...
    Returns
    -------
    dict with keys ``'real'``, ``'estimated_corr'``, ``'estimated_raw'``,
    ...
    for (finger, zone), grp in df.groupby(["finger", "zone"]):
        if grp["x_real"].notna().any():
            real[(finger, zone)] = (
                grp["x_real"].mean(), grp["y_real"].mean(),
            )
        est_corr[(finger, zone)] = (
            grp["x_est_corr"].mean(), grp["y_est_corr"].mean(),
        )
        est_raw[(finger, zone)] = (
            grp["x_est"].mean(), grp["y_est"].mean(),
        )
