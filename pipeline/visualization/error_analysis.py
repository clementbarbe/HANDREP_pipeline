"""Figure 02 — error bar charts and error vectors, with optional cm axis."""
...
def plot_error_analysis(analysis, output_path, cfg):
    ...
    ax.set_title("A) Mean error per finger")
    if cm is not None:
        ax2 = ax.secondary_yaxis(
            "right",
            functions=(lambda x: x * cm, lambda x: x / cm),
        )
    ...
    ax.set_xticklabels(stats_lm["label"], fontsize=9)
    ax.set_ylabel("Euclidean error (px)")
    ax.set_title("B) Error per landmark")
    if cm is not None:
        ax2 = ax.secondary_yaxis(
            "right",
            functions=(lambda x: x * cm, lambda x: x / cm),
    ...
    ax.scatter([], [], c="gray", marker="x", linewidths=2,
